...
{
    [Test]
    public async Task Handle_Should_Call_UpdateItems()
    {
        // Arrange
        var db = ArrangeDb; // however you set this up
        var character = new Character { Id = 1, UserId = 10 };
        db.Characters.Add(character);
        await db.SaveChangesAsync();

        var mockCharacterService = new Mock<ICharacterService>();
        mockCharacterService
            .Setup(s => s.UpdateItems(db, character, It.IsAny<IList<EquippedItemIdViewModel>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.NoErrors);

        var handler = new UpdateCharacterItemsCommand.Handler(db, Mapper, mockCharacterService.Object);

        var command = new UpdateCharacterItemsCommand
        {
            CharacterId = 1,
            Items = new List<EquippedItemIdViewModel> { new() { UserItemId = 123, Slot = ItemSlot.Head } },
        };
        ...
        mockCharacterService.Verify(
            s => s.UpdateItems(db, character, command.Items, It.IsAny<CancellationToken>()),
            Times.Once);
    }

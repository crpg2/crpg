...
public class GetMarketplaceOffersQueryTest : TestBase
{
    [Test]
    public async Task ShouldReturnOnlySellerOffersInSellerMode()
    {
        User user1 = new();
        User user2 = new();
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id),
            CreateOffer(user1.Id),
            CreateOffer(user2.Id));
        await ArrangeDb.SaveChangesAsync();

        var res = await new GetMarketplaceOffersQuery.Handler(ActDb, Mapper, Mock.Of<IDateTime>()).Handle(new GetMarketplaceOffersQuery
        {
            SellerId = user1.Id,
    ...
    public async Task ShouldIgnoreOtherFiltersInSellerMode()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, offeredGold: 0, requestedGold: 9999),
            CreateOffer(user1.Id, offeredGold: 150, requestedGold: 0),
            CreateOffer(user2.Id));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldOrderByCreatedAtDescending()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, createdAt: now.AddMinutes(-3)),
            CreateOffer(user2.Id, createdAt: now.AddMinutes(-1)),
            CreateOffer(user3.Id, createdAt: now.AddMinutes(-2)));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterByOfferedItemId()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, offeredUserItemId: userItem1.Id),
            CreateOffer(user2.Id, offeredUserItemId: userItem2.Id));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterByRequestedItemId()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedItemId: itemA.Id),
            CreateOffer(user2.Id, requestedItemId: itemB.Id));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterByRequestedItemRanks()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedItemId: itemRank1.Id),
            CreateOffer(user2.Id, requestedItemId: itemRank3.Id));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterByRequestedItemType()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedItemId: shield.Id),
            CreateOffer(user2.Id, requestedItemId: bow.Id));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterByOfferedItemRankAndTypeTogether()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, offeredUserItemId: userItem1.Id),
            CreateOffer(user2.Id, offeredUserItemId: userItem2.Id),
            CreateOffer(user3.Id, offeredUserItemId: userItem3.Id));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterByRequestedItemRankAndTypeTogether()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedItemId: rank3Bow.Id),
            CreateOffer(user2.Id, requestedItemId: rank3Shield.Id),
            CreateOffer(user3.Id, requestedItemId: rank2Bow.Id));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldClampPageSizeToOneHundredWhenPageSizeGreaterThanOneHundred()
    {
        ...
        for (int i = 0; i < 120; i++)
        {
            ArrangeDb.MarketplaceOffers.Add(CreateOffer(user.Id, createdAt: DateTime.UtcNow.AddMinutes(-i)));
        }
    ...
    public async Task ShouldFilterGoldNone()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedGold: 0),
            CreateOffer(user2.Id, requestedGold: 100));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterGoldCustomRange()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedGold: 0),
            CreateOffer(user2.Id, requestedGold: 25),
            CreateOffer(user3.Id, requestedGold: 200));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterHeirloomPointsNone()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedHeirloomPoints: 0),
            CreateOffer(user2.Id, requestedHeirloomPoints: 3));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldFilterHeirloomPointsCustomRange()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedHeirloomPoints: 0),
            CreateOffer(user2.Id, requestedHeirloomPoints: 3),
            CreateOffer(user3.Id, requestedHeirloomPoints: 10));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldNormalizeRangeWhenMinGreaterThanMax()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedGold: 4),
            CreateOffer(user2.Id, requestedGold: 7),
            CreateOffer(user3.Id, requestedGold: 12));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldClampNegativeRangeBoundsToZero()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(user1.Id, requestedGold: 0),
            CreateOffer(user2.Id, requestedGold: 1),
            CreateOffer(user3.Id, requestedGold: 6));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldNotApplyBuyerFilterWhenBuyerNotFound()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(seller1.Id, requestedGold: 999),
            CreateOffer(seller2.Id, requestedGold: 999));
        await ArrangeDb.SaveChangesAsync();
    ...
    [Test]
    public async Task ShouldExcludeBuyerOwnOffersInAffordableMode()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(buyer.Id, requestedGold: 1, requestedHeirloomPoints: 1),
            CreateOffer(seller.Id, requestedGold: 1, requestedHeirloomPoints: 1));
        await ArrangeDb.SaveChangesAsync();
    ...
    [Test]
    public async Task ShouldReturnOnlyAffordableOffersForBuyerResources()
    {
        ...
        ArrangeDb.MarketplaceOffers.AddRange(
            CreateOffer(seller1.Id, requestedGold: 60, requestedHeirloomPoints: 3),
            CreateOffer(seller2.Id, requestedGold: 101, requestedHeirloomPoints: 3),
            CreateOffer(seller3.Id, requestedGold: 60, requestedHeirloomPoints: 6));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldRequireBuyerOwningRequestedItem()
    {
        ...
        ArrangeDb.MarketplaceOffers.Add(CreateOffer(seller.Id, requestedGold: 10, requestedHeirloomPoints: 1, requestedItemId: "req_item"));
        await ArrangeDb.SaveChangesAsync();
    ...
    public async Task ShouldSkipItemOwnershipCheckWhenRequestedItemIsNull()
    {
        ...
        ArrangeDb.MarketplaceOffers.Add(CreateOffer(seller.Id, requestedGold: 10, requestedHeirloomPoints: 1, requestedItemId: null));
        await ArrangeDb.SaveChangesAsync();
    ...
    [Test]
    public async Task ShouldExcludeOfferWhenBuyerRequestedItemIsBroken()
    {
        User buyer = new() { Gold = 100, HeirloomPoints = 5 };
        User seller = new();
        ArrangeDb.Users.AddRange(buyer, seller);
        ...
        ArrangeDb.MarketplaceOffers.Add(CreateOffer(seller.Id, requestedGold: 10, requestedHeirloomPoints: 1, requestedItemId: "req_item_broken"));
        await ArrangeDb.SaveChangesAsync();
    ...
    [Test]
    public async Task ShouldExcludeOfferWhenBuyerRequestedItemIsPersonal()
    {
        User buyer = new() { Gold = 100, HeirloomPoints = 5 };
        User seller = new();
        ArrangeDb.Users.AddRange(buyer, seller);
        ...
        ArrangeDb.MarketplaceOffers.Add(CreateOffer(seller.Id, requestedGold: 10, requestedHeirloomPoints: 1, requestedItemId: "req_item_personal"));
        await ArrangeDb.SaveChangesAsync();
    ...
    [Test]
    public async Task ShouldExcludeOfferWhenBuyerRequestedItemIsClanBorrowed()
    {
        User buyer = new() { Gold = 100, HeirloomPoints = 5 };
        User seller = new();
        ArrangeDb.Users.AddRange(buyer, seller);
        ...
        ArrangeDb.MarketplaceOffers.Add(CreateOffer(seller.Id, requestedGold: 10, requestedHeirloomPoints: 1, requestedItemId: "req_item_borrowed"));
        await ArrangeDb.SaveChangesAsync();

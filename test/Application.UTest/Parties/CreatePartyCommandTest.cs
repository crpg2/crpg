...
public class CreatePartyCommandTest : TestBase
{
    ...
    {
        CreatePartyCommand.Handler handler = new(ActDb, Mapper, Mock.Of<ICampaignMap>(), Constants);
    ...
    public async Task ShouldReturnErrorIfAlreadyRegistered()
    {
        ...
        Assert.That(res.Errors, Is.Not.Null);
        Assert.That(res.Errors![0].Code, Is.EqualTo(ErrorCode.UserAlreadyRegisteredToCampaign));
    }

    [Test]
    public async Task ShouldRegisterToCampaign()
    {
        ...
        campaignMapMock.Setup(sm => sm.GetSpawnPosition(Region.Na)).Returns(new Point(150, 50));
        CreatePartyCommand.Handler handler = new(ActDb, Mapper, campaignMapMock.Object, Constants);
        var res = await handler.Handle(new CreatePartyCommand
        {
            UserId = user.Id,
        }, CancellationToken.None);

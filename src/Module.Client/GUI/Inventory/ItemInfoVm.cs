...
public class ItemInfoVM : ViewModel
{
            ...
            if (_clanArmory.GetCrpgUserItemArmoryStatus(UserItemExtended.Id, out var itemArmoryStatus))
            {
                ...
                switch (itemArmoryStatus)
                {
                    ...
                    case CrpgGameArmoryItemStatus.NotYoursAvailable:
                        ...
                        break;
                    case CrpgGameArmoryItemStatus.NotYoursBorrowed when isLeader || isOfficer:
    ...
    private void UpdateArmoryState(CrpgUserItemExtended? userItemEx)
    {
        ...
        ArmoryButtonText = status switch
        {
            ...
            CrpgGameArmoryItemStatus.NotYoursBorrowed => isOfficer || isLeader ? new TextObject("{=KC9dx159}Force Return").ToString() : new TextObject("{=KC9dx160}Unavailable").ToString(),
    ...
    private void AddWeaponTuples(CrpgItem crpgItem, ItemObject itemObj)
    {
        ...
        else if (iType == CrpgItemType.Thrown)
        {
            AddThrownWeaponTuples(crpgItem);
        }
    }
    ...
    private void AddThrownWeaponTuples(CrpgItem crpgItem)
    {
        var weapon = crpgItem.Weapons![0];

        Tuples.Add(new ItemInfoTupleVM { CategoryName = new TextObject("{=KC9dx171}Weight").ToString(), ValueText = crpgItem.Weight.ToString("F2") });
        AddWeaponStat(new TextObject("{=KC9dx193}Damage").ToString(), weapon.ThrustDamage, weapon.ThrustDamageType);
        Tuples.Add(new ItemInfoTupleVM { CategoryName = new TextObject("{=KC9dx230}Throw speed").ToString(), ValueText = weapon.ThrustSpeed.ToString() });
        Tuples.Add(new ItemInfoTupleVM { CategoryName = new TextObject("{=KC9dx197}Missile speed").ToString(), ValueText = weapon.MissileSpeed.ToString() });
        Tuples.Add(new ItemInfoTupleVM { CategoryName = new TextObject("{=KC9dx196}Accuracy").ToString(), ValueText = weapon.Accuracy.ToString() });
        Tuples.Add(new ItemInfoTupleVM { CategoryName = new TextObject("{=KC9dx195}Ammo").ToString(), ValueText = weapon.StackAmount.ToString() });

        // Melee mode — some thrown weapons (javelins, throwing axes) can also be swung.
        AddWeaponStat(new TextObject("{=KC9dx188}Swing damage").ToString(), weapon.SwingDamage, weapon.SwingDamageType, skipIfZero: true);
        AddWeaponStat(new TextObject("{=KC9dx189}Swing speed").ToString(), weapon.SwingSpeed, skipIfZero: true);
    }

...
public class InventoryGridVM : ViewModel
{
    ...
    internal event Action<InventorySection>? OnInventoryRefreshRequestedClick;
    ...
    internal bool UpdateTeamItemQuantity(string itemId, int quantity)
    {
    ...
    private void InitializeSortTypes()
    {
        InventorySortTypesLeft = new MBBindingList<InventorySortTypeVM>
        {
            new("onehanded", "ui_crpg_icon_white_onehanded", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.OneHandedWeapon, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("twohanded", "ui_crpg_icon_white_twohanded", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.TwoHandedWeapon, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("polearm", "ui_crpg_icon_white_polearm", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Polearm, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("shield", "ui_crpg_icon_white_shield", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Shield, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("thrown", "ui_crpg_icon_white_thrown", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Thrown, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("bow", "ui_crpg_icon_white_bow", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Bow, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("crossbow", "ui_crpg_icon_white_crossbow",
                slot => slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Crossbow, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("gun", "ui_crpg_icon_white_musket", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Musket ||
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Pistol, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("ammo", "ui_crpg_icon_white_arrow", slot =>
                ...
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Bullets, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
        };

        InventorySortTypesTop = new MBBindingList<InventorySortTypeVM>
        {
            new("headarmor", "ui_crpg_icon_white_headarmor", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.HeadArmor, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("cape", "ui_crpg_icon_white_cape", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Cape, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("chestarmor", "ui_crpg_icon_white_chestarmor", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.BodyArmor, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("handarmor", "ui_crpg_icon_white_handarmor", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.HandArmor, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("legarmor", "ui_crpg_icon_white_legarmor", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.LegArmor, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("mount", "ui_crpg_icon_white_mount", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Horse, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("mountarmor", "ui_crpg_icon_white_mountharness", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.HorseHarness, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
            new("banner", "ui_crpg_icon_white_flag", slot =>
                slot.ItemObj?.ItemType == ItemObject.ItemTypeEnum.Banner, true, OnSortTypeClicked, OnSortTypeAlternateClicked),
        };
    }
    ...
    private void ExecuteAlternateClickUserInventory()
    {
        if (ActiveSection == InventorySection.UserInventory)
        {
            OnInventoryRefreshRequestedClick?.Invoke(InventorySection.UserInventory);
        }
    }

    private void ExecuteAlternateClickArmory()
    {
        if (ActiveSection == InventorySection.Armory)
        {
            OnInventoryRefreshRequestedClick?.Invoke(InventorySection.Armory);
        }
    }

    private void ExecuteAlternateClickTeamInventory()
    {
        if (ActiveSection == InventorySection.TeamInventory)
        {
            OnInventoryRefreshRequestedClick?.Invoke(InventorySection.TeamInventory);
        }
    }
    ...
    private void OnSortTypeAlternateClicked(InventorySortTypeVM clickedSort)
    {
        foreach (var sort in InventorySortTypesLeft.Concat(InventorySortTypesTop))
        {
            sort.IsSelected = sort == clickedSort;
        }

        _activeFilters = [clickedSort];
        RefreshFilteredItemsFast();
        OnInventorySortTypeClicked?.Invoke(clickedSort);
    }
    ...
    public string UserInventoryButtonText { get; set => SetField(ref field, value, nameof(UserInventoryButtonText)); }
        = new TextObject("{=KC9dx136}User").ToString();
    [DataSourceProperty]
    ...
    public string TeamInventoryButtonText { get; set => SetField(ref field, value, nameof(TeamInventoryButtonText)); }
        = new TextObject("{=KC9dx138}Team").ToString();
    [DataSourceProperty]

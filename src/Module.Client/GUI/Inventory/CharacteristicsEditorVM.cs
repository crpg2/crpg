...
public class CharacteristicsEditorVM : ViewModel
{
    ...
    private void ExecuteClickApplyChanges()
    {
        ...
        if (now - _lastApiCharacteristicsApplyClick < _apiUsageClickCooldown)
        {
            ...
            CrpgHudNotificationUiHandler.AddToLog("statuslog", new CrpgHudNotificationOptions { Text = new TextObject("{=KC9dx118}Please wait to send another API request.").ToString(), Color = Colors.Red });
        ...
        if (!ValidateCharacteristicsBeforeApply(out string errorMessage))
        {
            ...
            CrpgHudNotificationUiHandler.AddToLog("statuslog", new CrpgHudNotificationOptions
            {
                Text = new TextObject("{=KC9dx119}Cannot apply: {ERROR_MESSAGE}").SetTextVariable("ERROR_MESSAGE", errorMessage).ToString(),
                Color = Colors.Red,
            });
            return;
        }
    ...
    {
        CrpgHudNotificationUiHandler.AddToLog("statuslog", new CrpgHudNotificationOptions { Text = new TextObject("{=KC9dx120}Unapplied changes reset to initial state.").ToString(), Color = Colors.White });
        ResetToInitialCharacteristics();
    }
    ...
    private void ExecuteClickRefreshApi()
    {
        ...
        CrpgHudNotificationUiHandler.AddToLog("statuslog", new CrpgHudNotificationOptions { Text = new TextObject("{=KC9dx121}Updating Characteristics from API.").ToString(), Color = Colors.Cyan });
    }

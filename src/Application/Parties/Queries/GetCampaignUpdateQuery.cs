...
public record GetCampaignUpdateQuery : IMediatorRequest<CampaignUpdate>
{
    public int PartyId { get; init; }

    internal class Handler(ICrpgDbContext db, IMapper mapper, ICampaignMap campaignMap, ICampaignSpeedModel campaignSpeedModel, ICampaignRouting campaignRouting) : IMediatorRequestHandler<GetCampaignUpdateQuery, CampaignUpdate>
    {
        ...
        public async ValueTask<Result<CampaignUpdate>> Handle(GetCampaignUpdateQuery req, CancellationToken cancellationToken)
        {
            ...
            double viewDistance = _campaignMap.ComputeViewDistance(currentTerrainType);
            ...
            var speed = _campaignSpeedModel.ComputePartySpeed(party, currentTerrainType);
        ...
        private List<PartyOrderPathSegmentViewModel> BuildOrderPathSegments(Point start, PartyOrderViewModel orderVm, Terrain[] terrains, double baseSpeed)
        {
            ...
            {
                var pathSegments = _campaignRouting.BuildPathSegments(points[i], points[i + 1], terrains);

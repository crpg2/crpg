...
/// </summary>
internal interface ICampaignSpeedModel
{
...
internal class CampaignSpeedModel(ICampaignRouting campaignRouting) : ICampaignSpeedModel
{
    ...
    public PartySpeed ComputePartySpeed(Party party, TerrainType? currentTerrainType)
    {
        double terrainInfluence = currentTerrainType.HasValue
            ? _campaignRouting.GetTerrainSpeedMultiplier(currentTerrainType.Value)
            : 1.0;
    ...
    private double MountsInfluence(float troops, List<ItemStack> partyItems)
    {
        ...
        foreach (ItemStack partyItem in partyItems.OrderByDescending(i => i.Item!.Mount!.HitPoints))
        {
            ...
            if (mounts >= troops && mountSpeed >= forcedMarchSpeed)
            {
                ...
                // hit points to calculate the speed, because the campaign is about sustained speed. Marathon runner are

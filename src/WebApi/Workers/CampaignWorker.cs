...
/// <summary>Worker to update the campaign map (movements, battles, ...).</summary>
public class CampaignWorker : BackgroundService
{
    private static readonly AssemblyName AssemblyName = typeof(CampaignWorker).Assembly.GetName();
    ...
    private static readonly ILogger Logger = LoggerFactory.CreateLogger<CampaignWorker>();
    // private static readonly TimeSpan TickInterval = TimeSpan.FromMinutes(1); // TODO: FIXME: To const
    ...
    public CampaignWorker(IServiceScopeFactory serviceScopeFactory)
    {
    ...
    {
        using var span = ActivitySource.StartActivity("campaign.tick");
    ...
    private Task SleepUntilNextTick(TimeSpan elapsed, CancellationToken cancellationToken)
    {
        ...
        Logger.LogWarning("Campaign tick took more time than expected: {0} (expected less than {1})",

...
internal class CrpgClanArmoryClient : MissionNetwork
{
    ...
    private readonly List<byte> _armoryChunkBuffer = [];
    ...
    internal void RequestArmoryAction(ClanArmoryActionType action, CrpgUserItemExtended? uItem, bool force = false)
    {
        ...
        if (!force && (now - _lastRequestTime < _requestCooldown || _outstandingRequest))
        {
            ShowStatusLog(new TextObject("{=KC9dx226}Armory -- please wait before trying again.").ToString(), Colors.Red);
        ...
        {
            Debug.Print($"RequestArmoryAction: clanId ({UserClanId}) or userId ({userId}) is invalid", 0, Debug.DebugColor.Red);
            return;
        }
    ...
    private void HandleArmoryActionResult(ServerSendArmoryActionResult message)
    {
        ...
        if (!message.Success)
        {
            ...
            ShowStatusLog(new TextObject("{=KC9dx227}Armory action failed: {ERROR}").SetTextVariable("ERROR", message.ErrorMessage).ToString(), Colors.Red);
            SoundEvent.PlaySound2D("event:/ui/notification/alert");
        }
    }
    ...
    private void HandleClanArmoryCompleteUpdate(ServerSendClanArmoryCompleteUpdate message)
    {
        ...
        {
            _armoryChunkBuffer.Clear();
        }

        _armoryChunkBuffer.AddRange(message.ChunkData);
        ...
        try
        {
            using System.IO.MemoryStream compressed = new(_armoryChunkBuffer.ToArray(), writable: false);
            ...
            var items = new List<CrpgClanArmoryItem>(count);
            ...
            SetClanArmoryItems(items);
        }
        catch (Exception ex)
        {
            Debug.Print($"[ArmoryClient] Decompression failed: {ex.Message}", 0, Debug.DebugColor.Red);
        }
        finally
        {
            _outstandingRequest = false;

<script setup lang="ts">
    ...
    import { computeSpeedStats, computeWeaponLengthMountPenalty } from '~/services/characters-service'
    ...
    const mountStatsPenalty = computed(() => {
        if (!props.longestWeaponLength || !props.characteristics.attributes.strength) return null
...
<template>
    ...
    <SimpleTableRow :label="$t('character.stats.mountedWeaponPenalty.title')"
                    :value="mountStatsPenalty ? $n(1 - mountStatsPenalty, 'percent') : '—'"
                    :tooltip="{
      title: $t('character.stats.mountedWeaponPenalty.title'),
      description: mountStatsPenalty
        ? $t('character.stats.mountedWeaponPenalty.desc')
        : 'No weapon equipped.',
    }" />
</template>

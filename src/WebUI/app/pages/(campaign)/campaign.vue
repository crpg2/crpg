...
import { useMap, useMapContextProvider } from '~/composables/campaign/use-map'
import { useParty, usePartyState } from '~/composables/campaign/use-party'
import { usePartyMove } from '~/composables/campaign/use-party-move'
import { usePartyOrder } from '~/composables/campaign/use-party-order'
import { useSettlements } from '~/composables/campaign/use-settlements'
import { useTerrains } from '~/composables/campaign/use-terrains'
import { TERRAIN_TYPE } from '~/models/campaign/terrain'
import { SomeRole } from '~/models/role'
import { getSelfUpdate, shouldPartyBeInBattle, shouldPartyBeInSettlement } from '~/services/campaign/party-service'
      ...
      try {
        ...
        if (!partyState.value) {
          ...
          if (partyRes?.errors !== null || partyRes.data === null) {
            return navigateTo({ name: 'join-to-campaign' })
          }
        ...
        if (shouldPartyBeInSettlement(party) && !to.meta.groups?.includes('campaignsettlement')) {
          return navigateTo({
        ...
        if (shouldPartyBeInBattle(party) && !to.meta.groups?.includes('campaignbattle')) {
          return navigateTo({
...
// TODO: need a name
const isBLocked = computed(() => route.name !== 'campaign')
</script>
     ...
     -->
      ...
      >
        <UFieldGroup size="xl">
          ...
          <UTooltip :text="$t('campaign.control.terrainLayerToggle')">
            <UButton
              variant="soft"
              color="neutral"
          ...
          <UTooltip :text="$t('campaign.control.searchBySettlements')">
            <UButton
              variant="soft"
              color="neutral"
